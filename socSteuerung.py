...
def find_sd_by_label_env(media_roots):
...
def find_sd_from_env(media_roots):
...
def atomic_write(path: Path, data: str, fsync: bool = False):
...
class DBusInterface:
    ...
    def get_value(self, service, path, default: Optional[float] = 0.0) -> Optional[float]:
    ...
    def get_raw_value(self, service, path, default: Optional[float] = None) -> Optional[float]:
    ...
    def coerce_dbus_value(self, value):
    ...
    def set_bus_item_value(self, obj, value):
...
class WinterController:
    ...
    def sd_should_override_ram(self, sd_data, ram_data):
        ...
        if not isinstance(ram_data, dict):
            return True
        return sd_data.get("ts", 0) > ram_data.get("ts", 0)

    def sd_persistent_keys(self):
        """Return the durable SD keys that are meaningful in the active season."""
        keys = list(SD_PERSISTENT_BASE_KEYS)
    ...
    def merge_sd_persistent_state(self, state, sd_data, ram_data=None):
        """Merge only explicit SD keys, never treating SD as a full controller state."""
        if not self.sd_should_override_ram(sd_data, ram_data):
            return
        for key in self.sd_persistent_keys():
            if key in sd_data:
                state[key] = copy.deepcopy(sd_data[key])

    def merge_sd_state_if_newer(self, state, sd_data, ram_data):
        """Merge SD subset and seed cache only when SD is newer than the RAM state."""
        if not isinstance(sd_data, dict):
            return False
        if self.sd_should_override_ram(sd_data, ram_data):
            self.merge_sd_persistent_state(state, sd_data, ram_data)
            self.init_sd_state_cache(sd_data)
            return True
        self.sd_last_signature = None
        self.sd_last_persist_ts = 0
        return False

    def merge_state(self, defaults, chosen):
    ...
    def save_state_to_ram(self, force_persist: bool = False):
    ...
    def backup_bad_state_file(self, path: Path):
    ...
    def refresh_sd_paths(self, force: bool = False):
    ...
    def persist_state_to_sd(self, force_persist: bool = False):
    ...
    def can_attempt_sd_write(self, force_persist: bool):
    ...
    def should_skip_sd_persist(self, now, signature, force_persist: bool):
    ...
    def init_sd_state_cache(self, sd_data):
        """Seed write-deduplication metadata from an already loaded SD state."""
        if not isinstance(sd_data, dict):
            return
        signature = {
            "max_charge_current_raw": sd_data.get("max_charge_current_raw"),
            "max_charge_current_raw_set": sd_data.get("max_charge_current_raw_set", False),
            "charge_current_owned_by_script": sd_data.get("charge_current_owned_by_script", False),
            "max_charge_current_script_last_set": sd_data.get("max_charge_current_script_last_set"),
        }
        if self.is_pv_history_window():
            signature["pv_history"] = sd_data.get("pv_history", [])
            signature["last_sample_date"] = sd_data.get("last_sample_date", "")
        if self.is_winter_window():
            signature["last_balance_ts"] = sd_data.get("last_balance_ts", 0)
            signature["last_balance_attempt_ts"] = sd_data.get("last_balance_attempt_ts", 0)
            signature["last_full_ts"] = sd_data.get("last_full_ts", 0)
        self.sd_last_signature = copy.deepcopy(signature)
        self.sd_last_persist_ts = sd_data.get("ts", 0)
        self.sd_pending_signature = None
        self.sd_pending_fsync = False

    def _enqueue_sd_write(self, payload_json, signature, fsync, sd_state_dir, sd_state_file):
    ...
    def _sd_writer_loop(self):  # pragma: no cover
    ...
    def process_next_sd_write(self):  # pragma: no cover
    ...
    def pop_sd_write_request(self):  # pragma: no cover
    ...
    def perform_sd_write(self, req):  # pragma: no cover
    ...
    def mark_sd_write_success(self, req, now):  # pragma: no cover
    ...
    def sync_pending_sd_metadata(self):  # pragma: no cover
    ...
    def mark_sd_write_failure(self, req, error, now):  # pragma: no cover
    ...
    def clear_sd_write_inflight(self):  # pragma: no cover
        """Mark the background writer as idle after one request attempt."""
        with self.sd_write_lock:
            self.sd_write_inflight = False

    def flush_sd_writes(self, timeout_seconds=5):
    ...
    def is_pv_history_window(self, now=None):
    ...
    def is_winter_window(self, now=None):
    ...
    def track_manual_min_soc_change(self, current_setting, now_ts, in_control_window):
    ...
    def remember_initial_min_soc(self, current_setting, last_seen):
    ...
    def same_min_soc(self, current_setting, last_seen):
        """Return True when two MinSoC readings are equivalent within tolerance."""
        return abs(current_setting - last_seen) <= MIN_SOC_EPSILON

    def was_recent_script_min_soc_write(self, current_setting, now_ts):
        ...
        return (
            ...
            and abs(current_setting - script_val) <= MIN_SOC_EPSILON
            and (now_ts - script_ts) <= MIN_SOC_SCRIPT_WRITE_MATCH_SECONDS
        )

    def register_manual_override_if_needed(self, current_setting, now_ts, in_control_window):
    ...
    def get_battery_service(self):
        ...
        if not self.battery_service_rescan_due():
            return None
        return self.scan_battery_service(cached)

    def battery_service_rescan_due(self):
    ...
    def scan_battery_service(self, cached):
    ...
    def store_best_battery_service(self, services, cached):
    ...
    def is_valid_battery_service(self, service):
    ...
    def select_preferred_battery_service(self, services):
    ...
    def select_best_battery_service(self, services):
    ...
    def get_battery_max_charge_current(self):
    ...
    def read_live_battery_max_current(self, service):
    ...
    def get_cached_battery_max_current(self):
    ...
    def get_house_load_power(self, grid_power_net=None, batt_power=None):
    ...
    def sum_phase_values(self, path_template):
    ...
    def compute_house_load_fallback(self, grid_power_net=None, batt_power=None):
    ...
    def clear_saved_max_charge_current_raw(self):
    ...
    def would_restrict_charge_current(self, current_raw, desired_a):
    ...
    def _same_charge_current(self, current_a, desired_a):
    ...
    def _write_max_charge_current(self, path, value):
    ...
    def _remember_script_charge_current(self, desired_a, force_persist: bool = False):
    ...
    def _restore_max_charge_current(self, path, current_raw, desired_a, reason, now):
    ...
    def _current_charge_ownership_valid(self, current_raw):
    ...
    def _ensure_charge_current_ownership(self, current_raw, desired_a):
    ...
    def _clamp_to_captured_charge_current(self, desired_a):
    ...
    def _charge_current_update_rate_limited(self, current_raw, desired_a, now):
    ...
    def get_normal_charge_current(self, battery_max_current=None):
    ...
    def first_normal_charge_current(self, candidates):
        """Return the first usable DVCC restore current from ordered candidates."""
        for current in candidates:
            if current is not None and (current < 0 or current > 0):
                return current
        return None

    def compute_charge_current_limit(self, house_load, battery_max_current, voltage):
    ...
    def available_grid_charge_power(self, house_load):
    ...
    def compute_safe_charge_current(self, battery_max_current):
    ...
    def clamp_to_normal_current(self, current, battery_max_current):
    ...
    def set_max_charge_current(self, desired_a, reason):
    ...
    def read_current_max_charge_current(self, max_curr_path):
    ...
    def can_apply_charge_limit(self, current_raw, desired_a):
    ...
    def write_charge_current_limit(self, max_curr_path, current_raw, desired_a, reason, now):
    ...
    def record_charge_current_write(self, desired_a, reason, now):
    ...
    def restore_normal_charge_current(self, battery_max_current):
    ...
    def maybe_log_status(self, house_load, battery_max_current):
    ...
    def build_status_message(self, house_load, battery_max_current, current_limit, now_ts):
    ...
    def update_full_soc_tracking(self, current_soc, now_ts):
    ...
    def track_full_soc_seconds(self, current_soc, now_ts, delta):
    ...
    def is_confirming_full_soc(self, current_soc):
    ...
    def add_full_soc_seconds(self, now_ts, delta):
        """Accumulate full-SoC time and persist once the confirmation threshold is met."""
        prev_full = self.state["full_soc_seconds"]
    ...
    def full_soc_threshold_crossed(self, previous_seconds):
        """Return True when continuous full SoC just crossed the confirmation threshold."""
        threshold_s = FULL_SOC_CONFIRM_MINUTES * 60
        return previous_seconds < threshold_s <= self.state["full_soc_seconds"]

    def reset_full_soc_seconds_if_needed(self):
    ...
    def track_balancing_progress(self, current_soc, now_ts, delta):
        """Track timeout and success conditions for an active balancing attempt."""
        if not self.state["balancing_active"]:
            return False

        if (now_ts - self.state["balancing_start_ts"]) > (BALANCING_MAX_HOURS * 3600):
    ...
    def update_balancing_full_seconds(self, current_soc, delta):
    ...
    def finish_balancing_attempt(self, now_ts, success: bool):
    ...
    def should_start_balancing(self, now_ts):
    ...
    def is_balancing_due(self, now_ts):
    ...
    def balancing_due_source_ts(self):
        """Return the timestamp from which the next balancing due time is measured."""
        if self.state["last_balance_ts"] > 0:
            return self.state["last_balance_ts"]
        if self.is_winter_window() and self.state["last_full_ts"] > 0:
            return self.state["last_full_ts"]
        return self.state["boot_ts"]

    def balancing_due_interval_seconds(self, due_source_ts):
    ...
    def is_balance_retry_cooling_down(self, now_ts):
        """Return True when a recent failed/started attempt is still cooling down."""
        elapsed = now_ts - self.state["last_balance_attempt_ts"]
        return elapsed < (BALANCING_RETRY_COOLDOWN_HOURS * 3600)

    def update_pv_history(self):
    ...
    def roll_pv_day(self, today_str, now_ts):
    ...
    def compute_completed_pv_average(self, now_ts):
        """Return the previous day's PV average when there is enough valid data."""
        if self.state.get("pv_time_s", 0) > 0:
            self.state["last_pv_integral_ts"] = now_ts
            return self.state.get("pv_energy_ws", 0.0) / self.state.get("pv_time_s", 1.0)

        samples = self.state.get("current_day_samples")
        if samples and self.is_pv_fallback_old_enough(now_ts):
            return sum(samples) / len(samples)
        return None

    def is_pv_fallback_old_enough(self, now_ts):
        """Return True when simple sample averaging is old enough to trust."""
        last_valid_ts = self.state.get("last_pv_integral_ts", 0.0)
        if last_valid_ts <= 0:
            last_valid_ts = self.state.get("boot_ts", now_ts)
        return (now_ts - last_valid_ts) >= (PV_FALLBACK_MIN_VALID_AGE_DAYS * 86400)

    def add_pv_history_value(self, avg):
    ...
    def collect_pv_sample(self, now_ts):
    ...
    def integrate_pv_sample(self, now_ts, pv_total):
        """Integrate PV power using trapezoidal area between adjacent samples."""
        last_ts = self.state.get("pv_last_sample_ts", 0.0)
        if last_ts <= 0:
            return
        dt = now_ts - last_ts
        if 0 < dt <= (LOOP_INTERVAL_SECONDS * 5):
            last_power = self.state.get("pv_last_sample_power", 0.0)
    ...
    def determine_target_soc(self, now_ts, current_soc=None):
    ...
    def mmdd_is_winter(self, current_date_val):
        """Return True when an MMDD integer falls inside the winter window."""
        return current_date_val >= WINTER_START_MMDD or current_date_val <= WINTER_END_MMDD

    def determine_transition_target(self, current_date_val):
    ...
    def determine_pre_winter_target(self):
    ...
    def determine_post_winter_target(self):
    ...
    def determine_winter_target(self, now_ts, current_soc=None):
    ...
    def should_use_winter_40_stage(self, current_soc):
    ...
    def start_balancing(self, now_ts):
    ...
    def transition_history_ready(self):
        """Return the recent PV history window only when it is complete."""
        hist = self.state["pv_history"]
        if len(hist) < TRANSITION_DAYS:
            return None
        return hist[-TRANSITION_DAYS:]

    def has_transition_history_below_threshold(self):
    ...
    def has_transition_history_above_threshold(self):
        """Return True when all recent transition PV values are above threshold."""
        hist = self.transition_history_ready()
        return hist is not None and all(val > PV_THRESHOLD for val in hist)

    def is_charge_control_active(self):
    ...
    def set_charge_state(self, active, paused):
    ...
    def expire_summer_override_if_needed(self, now_ts, in_control_window):
        """Expire a summer manual override after its hold window has elapsed."""
        manual_override_until = self.state.get("manual_override_until_ts", 0)
    ...
    def summer_override_active(self, now_ts, in_control_window):
        """Return True when a summer manual override is currently active."""
        return (not in_control_window) and self.state.get("manual_override_until_ts", 0) > now_ts

    def handle_active_summer_override(self, now_ts, state_changed):
    ...
    def log_active_summer_override_if_due(self, now_ts):
        """Periodically log remaining time for an active summer override."""
        last_override_log = self.state.get("last_manual_override_log_ts", 0)
        if (now_ts - last_override_log) < STATUS_LOG_INTERVAL_SECONDS:
            return False
        remaining_h = max((self.state["manual_override_until_ts"] - now_ts) / 3600.0, 0.0)
        self.dbus.log(f"Summer MinSoC override active ({remaining_h:.1f}h remaining)")
        self.state["last_manual_override_log_ts"] = now_ts
        return True

    def save_if_state_changed(self, state_changed):
        """Persist RAM state when a caller reports that state changed."""
        if state_changed:
            self.save_state_to_ram()

    def track_charge_deficit(self, needs_charge, now_ts):
        """Track how long the controller has been below target for window escalation."""
        current_start = self.state.get("charge_deficit_start_ts", 0)
    ...
    def charge_window_hours(self, now_ts):
        """Return the adaptive charge-window duration after unresolved deficit nights."""
        deficit_start = self.state.get("charge_deficit_start_ts", 0)
        if deficit_start <= 0:
        ...
        return CHARGE_WINDOW_BASE_HOURS * multiplier

    def is_charge_window_active(self, now, now_ts):
    ...
    def should_stage_charge_target(self, target_soc):
        """Return True for reserve-raising targets that should be reached in stages."""
        return target_soc > DEFAULT_SOC

    def _build_charge_context(self, current_soc, target_soc, current_setting, now, now_ts):
    ...
    def charge_time_ok(self, now, now_ts):
        """Return True when charging is allowed by window or active balancing."""
        return self.is_charge_window_active(now, now_ts) or self.state["balancing_active"]

    def import_only_power(self, grid_power_net):
        """Clamp net grid power to import-only watts."""
        return grid_power_net if grid_power_net > 0 else 0.0

    def effective_charge_control_active(self, needs_charge, target_soc, current_setting, now_ts):
    ...
    def boot_recover_active(self, needs_charge, target_soc, current_setting, now_ts):
    ...
    def _handle_charge_needed(self, current_limit_path, target_soc, current_soc, current_setting, context):
    ...
    def resume_soc_raise(self, current_limit_path, target_soc, current_setting, context):
    ...
    def log_soc_raise_resume_if_needed(self, target_soc, context):
    ...
    def set_min_soc_if_changed(self, current_limit_path, current_setting, target_soc):
    ...
    def apply_charge_current_policy(self, context):
    ...
    def apply_unstaged_target(self, current_limit_path, target_soc, current_setting, context):
    ...
    def finish_pause_soc_raise(self, context):
    ...
    def pause_soc_raise(self, current_limit_path, target_soc, current_soc, current_setting):
    ...
    def write_pause_soc_if_needed(self, current_limit_path, current_setting, pause_soc):
    ...
    def should_write_pause_soc(self, current_setting, pause_soc):
    ...
    def _handle_charge_not_needed(self, current_limit_path, target_soc, current_setting, battery_max_current):
    ...
    def apply_soc_logic(self, target_soc, current_soc):
        """
    ...
    def read_valid_min_soc_setting(self, current_limit_path):
    ...
    def log_invalid_min_soc_if_due(self):
        """Rate-limit logs for invalid or missing ESS MinSoC settings."""
        now = time.time()
        last_log = self.state.get("last_min_soc_invalid_log_ts", 0)
    ...
    def in_control_window(self, now):
        """Return True when seasonal control windows are active."""
        return self.is_winter_window(now) or self.is_pv_history_window(now)

    def enforce_default_min_soc_floor(self, current_limit_path, current_setting, target_soc):
    ...
    def dispatch_soc_logic(self, current_limit_path, target_soc, current_soc, current_setting, context):
    ...
    def read_current_soc(self):
        ...
        if current_soc is None or current_soc < 0 or current_soc > 100:
            now = time.time()
            last_log = self.state.get("last_soc_invalid_log_ts", 0)
            if (now - last_log) >= SOC_INVALID_LOG_INTERVAL_SECONDS:
    ...
    def log_mode_change(self, mode, target):
    ...
    def merge_sd_state(self, sd_data):
    ...
    def update_sd_window_state(self):
    ...
    def apply_sd_window_transition(self, sd_window_now):
